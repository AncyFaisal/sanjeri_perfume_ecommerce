...
from ..models.offer_models import ProductOffer, CategoryOffer, OfferApplication, SeasonalDiscount
...
def calculate_seasonal_discount(subtotal):
    """
    Calculate seasonal discounts based on SeasonalDiscount model rule.
    Applies the best active seasonal discount that the subtotal qualifies for.
    """
    now = timezone.now()
    best_discount = Decimal('0')
    
    # Get all active seasonal discounts
    active_discounts = SeasonalDiscount.objects.filter(
        is_active=True,
        valid_from__lte=now,
        valid_to__gte=now
    )
    
    for discount_rule in active_discounts:
        # Check minimum order rule
        if subtotal >= discount_rule.min_purchase_amount:
            # We pass the full subtotal instead of item_price
            discount_amount, _ = discount_rule.calculate_discount(subtotal)
            
            # Keep the largest discount if multiple are configured
