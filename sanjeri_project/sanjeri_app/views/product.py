...
def product_detail(request, product_id):
    ...
    try:
        ...
        is_available = (
            ...
            product.category.is_active and 
            not product.category.is_deleted
        )
        ...
        if request.user.is_authenticated:
            # TEMPORARY BYPASS FOR TESTING: normally we'd check the DB
            # has_purchased = OrderItem.objects.filter(
            #     order__user=request.user,
            #     order__status='delivered',
            #     variant__product=product
            # ).exists()
            has_purchased = True # Temporarily forcing True for testing
