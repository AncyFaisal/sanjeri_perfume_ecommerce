...
def men_products(request):
    ...
    if sort_by == 'best-selling':
        variants = variants.order_by('-product__is_best_selling', '-product__created_at')
        ...
        variants = variants.order_by('-product__is_featured', '-product__created_at')
    ...
    # Pagination
    paginator = Paginator(variants, 4)  # Changed to 4 products per page
...
def women_products(request):
    ...
    # Apply sorting - FIXED to match men's and unisex pages
    if sort_by == 'best-selling':
        variants = variants.order_by('-product__is_best_selling', '-product__created_at')
    elif sort_by == 'price-low-high':
        # Use effective_price annotation (like unisex page)
        variants = variants.annotate(
            effective_price=Coalesce('discount_price', 'price')
        ).order_by('effective_price')
    elif sort_by == 'price-high-low':
...
def brand_products(request, brand_name):
    ...
    sort_by = request.GET.get('sort', 'featured')
    if sort_by == 'best-selling':
        products = products.order_by('-is_best_selling', '-created_at')
    elif sort_by == 'price-low-high':
...
def product_search(request):
    ...
    if query:
        ...
        context = {
            ...
            'query': query,
            'page_obj': page_obj,
            'results_count': len(products) if isinstance(products, list) else products.count(),
            'sort_by': sort_by
        }
    else:
        context = {
