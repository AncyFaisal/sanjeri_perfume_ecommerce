...
try:
    ...
    js_search = "if (isInWishlist) {\n                            icon.className = 'fas fa-heart';\n                            icon.style.color = '#dc3545';\n                            button.classList.add('in-wishlist');\n                        }"
    js_replace = """if (isInWishlist) {
    ...
    # I'll also fix if the first file had different spacing:
    js_search2 = "if (isInWishlist) {                        icon.className = 'fas fa-heart';                        icon.style.color = '#dc3545';                        button.classList.add('in-wishlist');                    }"
    html = html.replace(js_search2, js_replace)

    card_html = """<div class="product-item position-relative">
              <!-- Volume Badge -->
    ...
    # We use string finding instead of regex to avoid backtracking
    for name in ["featured_products", "mens_products", "womens_products", "unisex_products", "search_products"]:
        start_tag = f"{{% for product in {name} %}}"
        end_tag = "{% empty %}"
        
        idx_start = html.find(start_tag)
        if idx_start == -1:
            print(f"Skipping {name}, not found.")
            # Maybe the regex partial matched earlier?
            start_tag_v = f"{{% for variant in {name} %}}"
            idx_start = html.find(start_tag_v)
            if idx_start == -1:
                continue
                
        idx_end = html.find(end_tag, idx_start)
        if idx_end == -1:
            print(f"Skipping {name}, empty tag not found.")
            continue
            
        replacement = f"{{% for variant in {name} %}}\n            <div class=\"swiper-slide\">\n              {card_html}\n            </div>\n            "
    ...
    print("Done patching successfully.")
